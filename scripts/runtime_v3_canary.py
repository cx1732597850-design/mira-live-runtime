...
def payload_from_row(row,mode):
    p={
      ...
      'baseline_provider':'CANARY_PINNED_2026_CORPUS','baseline_asof_et':'2026-08-29T18:45:00-04:00','baseline_source_id':'wnba_possessions_2026.parquet@511136104','baseline_source_hash':v2.SOURCES[2026][1],
    }
...
def main():
    ...
    for mode in v2.MODES:
      ...
      if out.get('betting_layer')!='DISABLED' or out.get('input_contract')!='PASS': raise RuntimeError('contract leak')
      # Exact numerical equivalence to v2 2023-2025 fit on the same feature row.
      models,feats,_=v2.fit_models(ds[ds.season.isin([2023,2024,2025])],mode,[2023,2024,2025])
      x=pd.DataFrame([{c:float(row[c]) for c in feats}])[feats]
      ...
      # Screenshot-friendly adapter path: FG + 3P, no exact possessions.
      ps=dict(p)
      for pref in ['a','b']:
    ...
    x=dict(base); x['pregame_games_min']=2; require_not_ready(v3.infer(x),'INSUFFICIENT_2026_PREGAME_BASELINE')
    ...
    results['negative_canaries']=['MISSING_REQUIRED_FEATURES','WINDOW_MISMATCH_PERIOD','WINDOW_MISMATCH_CLOCK','STALE_LIVE_STATE','TEAM_IDENTITY_MISMATCH','STATE_IDENTITY_MISMATCH_SCORE','STATE_IDENTITY_MISMATCH_DERIVED','INSUFFICIENT_2026_PREGAME_BASELINE','BASELINE_SOURCE_STALE','GAME_IDENTITY_MISMATCH']
